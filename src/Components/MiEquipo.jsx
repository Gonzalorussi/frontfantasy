...
function MiEquipo({ user, onTeamCreated }) {
  ...
  const colores = ['#e74c3c', '#3498db', '#2ecc71', '#f1c40f', '#9b59b6', '#1abc9c'];
  
  const handleConfirmarEquipo = async () => {
  // Validación básica
  if (!teamName || !selectedShield || !selectedFill || !primaryColor || !secondaryColor) {
  ...
  return (
    <div className="text-center mt-6">
      ...
      <h3>Elegí un escudo:</h3>
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        {shields.map(({id, Componente}) => (
          <button
            key={id}
            onClick={() => setSelectedShield(id)}
            className={`p-2 border rounded ${selectedShield === id ? 'border-blue-500' : ''}`}
          >
            <Componente className="w-32 h-32" style={{ color: selectedShield === id ? primaryColor : '#ccc' }}/>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        {rellenos.map(({id, Componente}) => (
          <button
            ...
            className={`p-2 border rounded ${selectedFill === id ? 'border-blue-500' : ''}`}
          >
            <Componente className="w-32 h-32" style={{ color: selectedFill === id ? secondaryColor : '#ccc' }}/>
          </button>
        ))}
      </div>

      <h3>Elegí colores del escudo:</h3>
          ...
          <div className="flex gap-2">
            {colores.map((color) => (
              <button
                ...
                style={{ backgroundColor: color, border: primaryColor === color ? '3px solid black' : 'none' }}
              />
            ))}
          </div>
        </div>
          ...
          <div className="flex gap-2">
            {colores.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full`}
                style={{ backgroundColor: color, border: secondaryColor === color ? '3px solid black' : 'none' }}
              />
            ))}
          </div>
        </div>
