...
export default function SeccionAlineacion({ roster, size = "normal" }) {
  ...
  return (
   <section className={`flex flex-col justify-center items-center min-h-[300px] bg-gray-900 p-6 rounded-2xl w-full ${isLarge ? "max-w-[900px]" : "max-w-[700px]"} shadow-lg`}>
      <div className={`mt-4 grid grid-cols-5 ${isLarge ? "gap-x-6 gap-y-8" : "gap-x-4 gap-y-6"}`}>
        {Object.keys(roster || {}).length > 0 ? (
          roles.map((rol) => {
            ...
            return (
              <div
                key={rol}
                className={`flex flex-col justify-center items-center ${isLarge ? "p-4 w-[150px] h-[240px]" : "p-2 w-[120px] h-[200px]"} bg-gray-800 rounded-xl shadow-lg`}
              >
                <div className="flex justify-center mb-3">
                  <Icono width={isLarge ? 50 : 40} height={isLarge ? 50 : 40} />
                </div>
                <p className="text-center text-lg text-gray-200 font-semibold mb-2 capitalize">{rol}</p>
                {jugador ? (
